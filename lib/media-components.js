    ...
    >
      ...
      <div className="flex items-center justify-between mt-6">
        <motion.button
          ...
          whileTap={{ scale: 0.9 }}
          onClick={prev}
          className="px-6 py-2 bg-pistach-500 text-white rounded-lg font-semibold hover:bg-pistach-600 transition-colors"
        >
          ← Previous
        </motion.button>
          ...
          {images.map((_, idx) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-all ${
                idx === current ? 'bg-pistach-500 w-8' : 'bg-gray-300'
              }`}
            />
          ))}
        </div>
