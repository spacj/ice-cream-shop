...
export function SectionHeader({ 
  ...
  return (
    ...
    >
      ...
      {subtitle && (
        <p className="text-lg text-taupe max-w-2xl mx-auto">{subtitle}</p>
      )}
    </motion.div>
  );
}
...
export function Badge({ children, variant = 'pistachio' }) {
  const variants = {
    ...
    red: 'bg-italian-red/10 text-italian-red',
...
export function TestimonialCard({ name, role, content, emoji = '🌿' }) {
  return (
    ...
    >
      ...
      <p className="text-taupe leading-relaxed italic">"{content}"</p>
    </motion.div>
  );
}
...
export function CTASection({ title, description, buttonText, buttonAction }) {
  return (
    <motion.section
      ...
      viewport={{ once: true }}
      className="py-20 px-4 bg-gradient-primary rounded-3xl text-white text-center"
    >
      ...
      {buttonText && (
        <PistachioButton 
          ...
          className="!bg-white !text-pistachio-600 hover:!bg-pistachio-50"
        >
          {buttonText}
        </PistachioButton>
      )}
